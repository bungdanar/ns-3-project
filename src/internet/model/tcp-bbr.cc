  ...
  static TypeId tid =
      TypeId ("ns3::TcpBbr")
          ...
          .AddAttribute ("HighGain", "Value of high gain", DoubleValue (2.89),
                         MakeDoubleAccessor (&TcpBbr::m_highGain), MakeDoubleChecker<double> ())
          ...
          .AddAttribute ("RttWindowLength", "Length of bandwidth windowed filter",
                         TimeValue (Seconds (10)), MakeTimeAccessor (&TcpBbr::m_rtPropFilterLen),
                         MakeTimeChecker ())
          .AddAttribute ("ProbeRttDuration", "Length of bandwidth windowed filter",
                         TimeValue (MilliSeconds (200)),
                         MakeTimeAccessor (&TcpBbr::m_probeRttDuration), MakeTimeChecker ());
  return tid;
}
...
void
TcpBbr::PrintToFile (Ptr<const TcpSocketState> tcb)
{
  ...
  if (!startPrintToFile)
    {
      ...
      std::ofstream traceAll ("TcpBbr_trace_for_node_" + std::to_string (node->GetId ()) + ".data",
...
TcpBbr::SetCwnd (Ptr<TcpSocketState> tcb, const struct RateSample *rs)
{
  ...
  //commented by Imtiaz Mahmud (imtiaz.tee@gmail.com) to comply with BBR 1.0
  /*
  ...
  */
  ...
  //added by Imtiaz Mahmud (imtiaz.tee@gmail.com) to comply with BBR 1.0
  ModulateCwndForRecovery (tcb, rs);
