...
NS_LOG_COMPONENT_DEFINE ("TcpBbrAcd");
NS_OBJECT_ENSURE_REGISTERED (TcpBbrAcd);

const double TcpBbrAcd::PACING_GAIN_CYCLE[] = {5.0 / 4, 3.0 / 4, 1, 1, 1, 1, 1, 1};

TypeId
TcpBbrAcd::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::TcpBbrAcd")
          .SetParent<TcpCongestionOps> ()
          .AddConstructor<TcpBbrAcd> ()
          .SetGroupName ("Internet")
          .AddAttribute ("HighGain", "Value of high gain", DoubleValue (2.89),
                         MakeDoubleAccessor (&TcpBbrAcd::m_highGain), MakeDoubleChecker<double> ())
          .AddAttribute ("BwWindowLength", "Length of bandwidth windowed filter",
                         UintegerValue (10),
                         MakeUintegerAccessor (&TcpBbrAcd::m_bandwidthWindowLength),
                         MakeUintegerChecker<uint32_t> ())
          .AddAttribute ("RttWindowLength", "Length of bandwidth windowed filter",
                         TimeValue (Seconds (10)), MakeTimeAccessor (&TcpBbrAcd::m_rtPropFilterLen),
                         MakeTimeChecker ())
          .AddAttribute ("ProbeRttDuration", "Length of bandwidth windowed filter",
                         TimeValue (MilliSeconds (200)),
                         MakeTimeAccessor (&TcpBbrAcd::m_probeRttDuration), MakeTimeChecker ());
  return tid;
}

TcpBbrAcd::TcpBbrAcd () : TcpCongestionOps ()
{
  NS_LOG_FUNCTION (this);
  m_uv = CreateObject<UniformRandomVariable> ();
}

TcpBbrAcd::TcpBbrAcd (const TcpBbrAcd &sock)
...
void
TcpBbrAcd::PrintToFile (Ptr<const TcpSocketState> tcb)
{
  ...
  if (!startPrintToFile)
    {
      ...
      std::ofstream traceAll ("TcpBbrAcd_trace_for_node_" + std::to_string (node->GetId ()) +
                                  ".data",
                              std::ofstream::out | std::ofstream::trunc);
    }

  std::ofstream traceAll ("TcpBbrAcd_trace_for_node_" + std::to_string (node->GetId ()) + ".data",
...
int64_t
TcpBbrAcd::AssignStreams (int64_t stream)
{
...
void
TcpBbrAcd::InitPacingRate (Ptr<TcpSocketState> tcb)
{
...
void
TcpBbrAcd::HandleRestartFromIdle (Ptr<TcpSocketState> tcb, const RateSample *rs)
{
...
void
TcpBbrAcd::SetPacingRate (Ptr<TcpSocketState> tcb, double gain)
{
...
uint32_t
TcpBbrAcd::InFlight (Ptr<TcpSocketState> tcb, double gain)
{
  NS_LOG_FUNCTION (this << tcb << gain);
...
bool
TcpBbrAcd::IsNextCyclePhase (Ptr<TcpSocketState> tcb, const struct RateSample *rs)
{
  NS_LOG_FUNCTION (this << tcb << rs);
  ...
  if (m_pacingGain == 1)
    ...
    {
      return isFullLength &&
             (rs->m_packetLoss > 0 || rs->m_priorInFlight >= InFlight (tcb, m_pacingGain));
    }
  else
    {
...
void
TcpBbrAcd::CheckCyclePhase (Ptr<TcpSocketState> tcb, const struct RateSample *rs)
{
  NS_LOG_FUNCTION (this << tcb << rs);
...
void
TcpBbrAcd::CheckFullPipe (const struct RateSample *rs)
{
...
void
TcpBbrAcd::CheckDrain (Ptr<TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this << tcb);
...
void
TcpBbrAcd::UpdateRTprop (Ptr<TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this << tcb);
...
void
TcpBbrAcd::SaveCwnd (Ptr<const TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this << tcb);
...
void
TcpBbrAcd::RestoreCwnd (Ptr<TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this << tcb);
...
void
TcpBbrAcd::HandleProbeRTT (Ptr<TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this << tcb);
...
void
TcpBbrAcd::CheckProbeRTT (Ptr<TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this << tcb);
...
void
TcpBbrAcd::SetSendQuantum (Ptr<TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this << tcb);
...
void
TcpBbrAcd::UpdateTargetCwnd (Ptr<TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this << tcb);
...
void
TcpBbrAcd::ProbeCongestion (Ptr<TcpSocketState> tcb)
{
  if (tcb->m_lastRtt.Get ().GetMicroSeconds () > tcb->m_minRtt.GetMicroSeconds ())
    {
      if (tcb->m_lastRtt.Get ().GetMicroSeconds () >= savedLastRtt - 1000 &&
          tcb->m_lastRtt.Get ().GetMicroSeconds () <= savedLastRtt + 1000)
        {
          countIfSteadyRtt++;
        }
      else
  ...
  savedLastRtt = tcb->m_lastRtt.Get ().GetMicroSeconds ();

  if (countIfSteadyRtt >= 2 ||
      tcb->m_lastRtt.Get ().GetMicroSeconds () > 2 * tcb->m_minRtt.GetMicroSeconds ())
    {
      probeCongestion = true;
    }

  probeCongestion = false;
}

void
TcpBbrAcd::LimitCwndToStopLoss (Ptr<TcpSocketState> tcb)
{
  if (probeCongestion || tcb->m_congState == TcpSocketState::CA_RECOVERY)
    {
      m_cWndGain = 1;
      UpdateTargetCwnd (tcb);

      tcb->m_cWnd =
          std::max (tcb->m_cWnd.Get (), tcb->m_bytesInFlight.Get () + tcb->m_lastAckedSackedBytes);
      tcb->m_cWnd = std::min (tcb->m_cWnd.Get () / 2, m_targetCWnd);
      tcb->m_cWnd = std::max (tcb->m_cWnd.Get (), m_minPipeCwnd);

      modifiedByLimitCwndToStopLoss = true;

      std::cout << "\n\n\n\n\n\n\n"
                << Simulator::Now ().GetSeconds () << ": node: " << this->GetNode ()->GetId ()
                << ": isCongestion - Congestion limit applied\n\n\n\n\n";
    }
  else
    modifiedByLimitCwndToStopLoss = false;
}

//Imtiaz Mahmud's (imtiaz.tee@gmail.com) implementation ends

void
TcpBbrAcd::ModulateCwndForRecovery (Ptr<TcpSocketState> tcb, const struct RateSample *rs)
{
  NS_LOG_FUNCTION (this << tcb << rs);
  ...
  if (rs->m_packetLoss > 0)
    {
      tcb->m_cWnd =
          std::max ((int) tcb->m_cWnd.Get () - (int) rs->m_packetLoss, (int) tcb->m_segmentSize);
    }
  /*
 ...
 */
  ...
  LimitCwndToStopLoss (tcb); //Imtiaz Mahmud's (imtiaz.tee@gmail.com) implementation
}

void
TcpBbrAcd::ModulateCwndForProbeRTT (Ptr<TcpSocketState> tcb)
{
  NS_LOG_FUNCTION (this << tcb);
  if (m_state == BbrMode_t::BBR_PROBE_RTT)
    {
...
void
TcpBbrAcd::SetCwnd (Ptr<TcpSocketState> tcb, const struct RateSample *rs)
{
  NS_LOG_FUNCTION (this << tcb << rs);
  UpdateTargetCwnd (tcb);
  ...
  if (!modifiedByLimitCwndToStopLoss && !modifiedByLimitCwndToStopLoss)
    {
      if (m_isPipeFilled)
        {
          tcb->m_cWnd =
              std::min (tcb->m_cWnd.Get () + (uint32_t) tcb->m_lastAckedSackedBytes, m_targetCWnd);
        }
      else if (tcb->m_cWnd < m_targetCWnd ||
               tcb->m_delivered < tcb->m_initialCWnd * tcb->m_segmentSize)
        {
...
void
TcpBbrAcd::UpdateRound (Ptr<TcpSocketState> tcb, const struct RateSample *rs)
{
  NS_LOG_FUNCTION (this << tcb << rs);
...
void
TcpBbrAcd::UpdateBtlBw (Ptr<TcpSocketState> tcb, const struct RateSample *rs)
{
  NS_LOG_FUNCTION (this << tcb << rs);
...
void
TcpBbrAcd::UpdateModelAndState (Ptr<TcpSocketState> tcb, const struct RateSample *rs)
{
  NS_LOG_FUNCTION (this << tcb << rs);
  UpdateBtlBw (tcb, rs);
...
void
TcpBbrAcd::UpdateControlParameters (Ptr<TcpSocketState> tcb, const struct RateSample *rs)
{
  NS_LOG_FUNCTION (this << tcb << rs);
...
std::string
TcpBbrAcd::WhichState (BbrMode_t mode) const
{
  switch (mode)
    {
    case 0:
...
void
TcpBbrAcd::SetBbrState (BbrMode_t mode)
{
  NS_LOG_FUNCTION (this << mode);
  NS_LOG_DEBUG (Simulator::Now () << " Changing from " << WhichState (m_state) << " to "
                                  << WhichState (mode));
  m_state = mode;
}

uint32_t
TcpBbrAcd::GetBbrState ()
{
  NS_LOG_FUNCTION (this);
  return m_state;
}
...
std::string
TcpBbrAcd::GetName () const
{
  return "TcpBbrAcd";
}

bool
TcpBbrAcd::HasCongControl () const
{
  NS_LOG_FUNCTION (this);
  return true;
}
...
void
TcpBbrAcd::CongControl (Ptr<TcpSocketState> tcb, const struct RateSample *rs)
{
  NS_LOG_FUNCTION (this << tcb << rs);
...
void
TcpBbrAcd::CongestionStateSet (Ptr<TcpSocketState> tcb,
  ...
  if (newState == TcpSocketState::CA_OPEN && !m_isInitialized)
    {
      ...
      m_sendQuantum = 1 * tcb->m_segmentSize;
      m_maxBwFilter = MaxBandwidthFilter_t (
          m_bandwidthWindowLength,
          DataRate (tcb->m_initialCWnd * tcb->m_segmentSize * 8 / m_rtProp.GetSeconds ()), 0);
      InitRoundCounting ();
      InitFullPipe ();
      EnterStartup ();
      InitPacingRate (tcb);
...
void
TcpBbrAcd::CwndEvent (Ptr<TcpSocketState> tcb, const TcpSocketState::TcpCAEvent_t event)
{
...
uint32_t
TcpBbrAcd::GetSsThresh (Ptr<const TcpSocketState> tcb, uint32_t bytesInFlight)
{
...
TcpBbrAcd::Fork (void)
{
  return CopyObject<TcpBbrAcd> (this);
}
